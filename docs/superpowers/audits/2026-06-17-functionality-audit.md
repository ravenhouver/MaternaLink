...
| /medicine-needs/[medicine] | MedicationDetailContent | mutation_gap | major | detail, history, stock prediction, timestamp, and remove action use APIs; save remains unavailable because page has no editable fields | save needs durable medicine edit form | partial | B | batch_b_partial | Remove from Inventory now calls DELETE `/master/obat/:id` and surfaces backend conflict errors; save remains explicit unavailable notice. |
